...
exports.create = function (req, res, next) {
  if (typeof req.body.reCaptcha === 'undefined' || req.body.reCaptcha.toString().length === 0) {
    return validationError(res, {captcha: ['Captcha Error']});
  } else {
    reCaptcha.go(req.body.reCaptcha, function (error, httpResponse, body) {
      body = JSON.parse(body);
      if (error) {
        return res.status(500).send('Error validating reCaptcha Token');
      }
      if (body.success) {
        ...
        return validationError(res,  { captcha:['Invalid Captcha']});
      }
    });
  }
};
